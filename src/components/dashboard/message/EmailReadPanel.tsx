...
const EmailReadPanel = ({data, setSelectedThread}: any) => {
   return (
      <div className="email-read-panel">
         {data?.map((item) => (
            <div key={item.id} className={`email-list-item ps-3 pe-3 ps-xxl-4 pe-xxl-4`} onClick={()=> setSelectedThread(item)}>
               <div className="email-short-preview position-relative">
                  <div className="d-flex align-items-center justify-content-between">
                     <div className="sender-name">{item.name}</div>
                     <div className="date">{moment(item?.created_at).startOf('minute').fromNow()}</div>
                  </div>
                  <div className="mail-sub">{item?.title}</div>
                  <div className="mail-text">{item?.message}</div>
